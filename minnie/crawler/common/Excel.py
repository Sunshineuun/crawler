...
class WriteXLSX(object):
    ...
    def write(self, _dbname, _tname, title):
        if not title:
            title = self.get_title(_dbname, _tname)
    ...
    def write1(self, _dbname, _tname, title):
        _title = [k for k, v in title.items()]
        self.write_title(_title)
        ...
        for data in self.mongo[_dbname][_tname].find():
            ...
            for _t in _title:
                column += 1
                for k1 in title[_t]:
                    # 如果当前title下的别名在数据中，那么就取值
                    if k1 in data:
                        # write的第一个,第二个参数时坐标, 第三个是要写入的数据
                        value, count = re.compile('【[\u4e00-\u9fa5]{0,}】').subn('', data[k1])
                        self.sheet.write(row, column, value, self.format)
                        break
        self.workbook.close()

    def write2(self, params):
        row = -1
        for data in params['data']:
            ...
            for d in str(data).split('#'):
                column += 1
                self.sheet.write(row, column, d, self.format)
        self.workbook.close()
