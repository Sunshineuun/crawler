...
class yaozh_monitored(yaozh):
    ...
    def _init_url(self):
        ...
        # 2016-12 分地市查找
        city = ['青海红十字医院', '省中医院', '省藏医院', '省妇女儿童医院', '省心脑血管病医院', '省第四人民医院', '西宁市第一人民医院',
                '西宁市市第二人民医院',
                '互助县人民医院']
        for c in city:
            result_list.append({
                'url': 'https://db.yaozh.com/monitored?p=1&area={area}&pageSize=30&time=2016-12'.format(
                    area=c),
                'type': self._cn_name
            })
        ...
        for c in city:
            result_list.append({
                'url': 'https://db.yaozh.com/monitored?p=1&area={area}&pageSize=30&time=2018-01'.format(
                    area=c),
                'type': self._cn_name
            })
...
class yaozh_yaopinzhongbiao(BaseCrawler):
    ...
    def _init_url(self):
        ...
        csv_reader = csv.reader(open('D:\Temp\药品通用名.csv', encoding='utf-8'))
        for row in csv_reader:
            result_list.append({
                'url': url.format(name=row[0]),
                'type': self._get_cn_name(),
